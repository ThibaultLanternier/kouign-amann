...
class TestPictureGroup(unittest.TestCase):
    ...
    def setUp(self):
        self._picture_group_not_grouped: PictureGroup = PictureGroup(
            [
                FakePicturePath(
                    folder_path=Path("root/NOT_GROUPED"),
                    day=date(2023, 10, 1),
                    hash="hash1",
                ),
                FakePicturePath(
        ...
        self._picture_group_partly_grouped = PictureGroup(
            [
                ...
                FakePicturePath(
                    folder_path=Path("root/EVENT-XXX"),
                    day=date(2023, 10, 3),
                    hash="hash2",
                ),
                FakePicturePath(
                    folder_path=Path("root/EVENT-YYY"),
                    day=date(2023, 10, 5),
                    hash="hash3",
                ),
                FakePicturePath(
    ...
    def test_list_pictures_to_move(self):
        self.assertEqual(
            self._picture_group_partly_grouped.list_pictures_to_move(),
            [
    ...
    def test_list_pictures_to_move_new_folder(self):
        self.assertEqual(
            self._picture_group_not_grouped.list_pictures_to_move(),
            [
                (
...
class TestPictureGrouper(unittest.TestCase):
    def setUp(self):
        self._picture_paths = [
            ...
            FakePicturePath(
                folder_path=Path("root/EVENT_1"), day=date(2023, 10, 2), hash="hash2"
            ),
            FakePicturePath(
                folder_path=Path("root/NOT_GROUPED"),
                day=date(2023, 10, 4),
            ...
            FakePicturePath(
                folder_path=Path("root/EVENT_2"), day=date(2023, 10, 5), hash="hash4"
            ),
        ]
