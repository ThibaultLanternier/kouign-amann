...
class TestPicturePathManager(unittest.TestCase):
    ...
    def setUp(self):
        picture_path_1 = FakePicturePath(
            folder_path=Path('day/2024-12-08'), 
            day=date(2024,12,8), 
            hash='xxx'
        )

        self._path_manager = PicturePathManager([picture_path_1], root_folder=Path('root_folder'))
    ...
    def test_picture_path_manager_should_regroup(self):
        self.assertEqual(self._path_manager._by_day, {date(2024,12,8): Path('day/2024-12-08')})

        self._path_manager.add_picture_path(
            FakePicturePath(
                folder_path=Path('day/2024-12-10'), 
                day=date(2024,12,10), 
                hash='yyy'
            )
        )

        self.assertEqual(self._path_manager._by_day, {
        ...
        self._path_manager.add_picture_path(
            FakePicturePath(
                folder_path=Path('day/2024-12-09'), 
                day=date(2024,12,9), 
                hash='yyy'
            )
        )
    ...
    def test_picture_path_manager_should_link_adjacent_days(self):
        ...
        # Continuous serie for December 8th, 9th and 10th, December 10th becomes linked with December 8th
        self.assertEqual(
            self._path_manager.get_folder_path(date(2024,12,8)),
            Path('day/2024-12-08')
        )
