...
from app.entities.picture_data import NotStandardFileNameException, PictureData, iPictureData
from app.repositories.picture_data import PictureDataRepository
from app.services.file import FileService, iFileService
from app.services.picture_id import PictureIdComputeException, PictureIdService, iPictureIdService
    ...
    def group(self, picture_list: list[Path]):
        ...
        for picture_path in picture_list:
            try:
                ...
                try:
                    picture_data = self._picture_id_service.compute_id(picture_path=picture_path)
        ...
        picture_group_list = picture_group.get_group_list(picture_list=picture_data_list)
        ...
        self._logger.info(f"Found {len(pictures_to_move)} pictures that need to be moved")
        ...
        for picture in pictures_to_move:
            self._file_service.move(
                origin_path=picture[0], target_path=picture[1]
            )
            progress_bar_count = progress_bar_count + 1
            progress_bar.update(progress_bar_count)


def group_use_case_factory(backup_folder_path: Path, hours_btw_pictures: int) -> GroupUseCase:
