...
class BackupUseCase:
    def __init__(self, file_service: iFileService, picture_id_service: iPictureIdService):
    ...
    def _backup_picture(self, picture_path: Path, strict_mode: bool) -> bool:
        ...
        if not strict_mode:
            picture_data = self._picture_id_service.get_from_cache(picture_path=picture_path)

        if picture_data is None:
            try:                
                picture_data = self._picture_id_service.compute_id(picture_path=picture_path)
            except PictureIdComputeException as e:
                self._logger.warning(f"Failed to compute picture id for {picture_path}: {e}")   
                return False
        
        self._file_service.backup(
    ...
    def backup(self, picture_list_to_backup: list[Path], strict_mode: bool = False) -> bool:
