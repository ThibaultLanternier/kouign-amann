...
class PictureGroup:
    def __init__(self, picture_path_list: list[abstractPicturePath]):
        ...
        self._picture_path_count: dict[Path, int] = {}
        ...
        for picture_path in self._picture_path_list:
            ...
            if folder_name != "NOT_GROUPED":
                ...
                self._picture_path_count[folder_path] = (
                    self._picture_path_count[folder_path] + 1
                )
        ...
        self._folder_list = [
            k
            for (k, v) in sorted(
                self._picture_path_count.items(), key=picture_count, reverse=True
            )
        ]
        ...
        if len(self._folder_list) == 0:
            ...
            self._folder_list = [
                root_folder
                / Path(f"{picture_path_list[0].get_day()} <EVENT_DESCRIPTION>")
            ]
...
class PictureGrouper:
    ...
    def group_pictures(self, max_days: int = 1) -> list[list[abstractPicturePath]]:
        sorted_picture_path_list = sorted(
            self._picture_path_list, key=lambda x: x.get_day()
        )
